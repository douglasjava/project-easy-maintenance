...
Valida a divisão de comissão de um afiliado entre N beneficiários (ex.: "Grupo Silva" + vendedor que
...
### C4 — Configurar split de 2 beneficiários somando 100%

| Passo | Ação | Resultado esperado |
|-------|------|---------------------|
| 1 | Em `/private/admin/affiliates`, clicar "Dividir comissão" na linha do Grupo Silva | Modal abre, vazio (2 linhas em branco, nenhum split configurado ainda) |
| 2 | Preencher linha 1: "Grupo Silva" / 66,67% — linha 2: "Vendedor João" / 33,33% | Texto "Soma atual: 100%" aparece em cinza (não vermelho) |
| 3 | Clicar "Salvar" | Toast de sucesso ("Divisão de comissão salva"), modal fecha |
| 4 | Reabrir "Dividir comissão" no mesmo afiliado | Modal carrega as 2 linhas já preenchidas com os valores salvos (66,67% e 33,33%) |
...
| Passo | Ação | Resultado esperado |
|-------|------|---------------------|
| 1 | Com o split de C4 configurado (66,67% / 33,33%) e uma comissão de R$150,00 gerada (C2) | — |
| 2 | Abrir `/private/admin/financials`, mês corrente | Linha "Grupo Silva" mostra o total (R$150,00); logo abaixo, 2 sub-linhas indentadas: "↳ Grupo Silva" (~R$100,00) e "↳ Vendedor João" (~R$49,99) |
| 3 | Conferir que a soma das sub-linhas bate (aproximadamente, ver nota de truncamento) com o total da linha principal | R$100,00 + R$49,99 = R$149,99 — 1 centavo de diferença é esperado (truncamento, já documentado na TASK-207, não é bug) |
...
- [ ] C1: suíte automatizada (backend + frontend) sem regressão
- [ ] C2: comissão de teste gerada via simulador, aparece no breakdown
- [ ] C3: afiliado sem split — nenhuma mudança visual ou de comportamento
