...
| Passo | Ação | Resultado esperado |
|-------|------|---------------------|
| 1 | Forçar uma condição de falha do complemento de IA (ex.: esgotar os créditos de IA do usuário de teste antes de gerar, ou desligar a internet durante o polling) | O catálogo (Etapa 2) continua exibido e utilizável normalmente |
...
- [ ] C1: `norm_segments` com contagem correta por segmento, nenhum item de `norms` órfão
- [ ] C2: catálogo abre instantâneo, sem chamada de IA, badges corretos
- [ ] C3: segmento OUTROS não quebra — vazio sem descrição, só IA com descrição
- [ ] C4: IA complementa corretamente, sem substituir os itens de catálogo já exibidos
- [ ] C5: IA não duplica item já coberto pelo catálogo, mesmo quando a descrição o menciona
